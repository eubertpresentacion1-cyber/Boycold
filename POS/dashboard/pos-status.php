...
if ($orderId > 0) {
    ...
    $stmt->bind_param("ii", $orderId, $branchId);
} else {
    $stmt = $connect->prepare(
        "SELECT o.*, u.phone AS user_phone
         FROM orders o
         LEFT JOIN users u ON u.user_name = o.user_name
         WHERE o.status IN ('pending', 'confirmed', 'preparing', 'ready', 'delivered')
         AND o.branch_id = ?
...
if ($pendingStmt) {
    $pendingStmt->bind_param("i", $branchId);
    $pendingStmt->execute();
...
$nextStatusMap = [
    ...
    'preparing' => ['status' => 'ready', 'label' => 'Mark as Out for Delivery'],
    'ready'     => ['status' => 'delivered', 'label' => 'Mark as Delivered'],
    'delivered' => ['status' => 'completed', 'label' => 'Mark as Completed'],
];

$prevStatusMap = [
    'preparing' => ['status' => 'confirmed', 'label' => 'Back to Confirmed'],
    'ready'     => ['status' => 'preparing', 'label' => 'Back to Preparing'],
    'delivered' => ['status' => 'ready', 'label' => 'Back to Out for Delivery'],
];

function pos_status_step_class(bool $reached, bool $current): string
{
    if ($current) {
        return 'status-step current';
    }

    return $reached ? 'status-step completed' : 'status-step';
}

$orderStatus = $order['status'] ?? '';
$createdAt = $order ? new DateTime($order['created_at']) : null;
$typeKey = $order['order_type'] ?? 'delivery';
$typeCode = $typeCodes[$typeKey] ?? 'GEN';
$orderNumber = $order ? sprintf('ONL-%s-%s-%05d', $typeCode, $createdAt->format('Y'), (int) $order['id']) : 'No order';
...
<body>
    ...
    <div class="app-shell">
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="sidebar-nav">
                <ul>
                    ...
                    <li>
                        <a href="pos-status.php" class="active">
        ...
        <div class="main-panel">
            ...
            <main class="status-workspace">
                <article class="order-status-card" aria-labelledby="order-status-title">
                    <?php if (!$order): ?>
                        <header class="order-card-header">
                            <div>
                                <p class="section-label">Order Details</p>
                                <p class="order-label">Order No.</p>
                                <h1 id="order-status-title">No active order</h1>
                            </div>
                        </header>
                        ...
                        <div class="status-alert" role="status">
                            ...
                            <div>
                                ...
                                <span>Accept an order from the popup or choose an active online order.</span>
                            </div>
                        </div>
                    ...
                    <?php else: ?>
                        <header class="order-card-header">
                            <div>
                                <p class="section-label">Order Details</p>
                                <p class="order-label">Order No.</p>
                                <h1 id="order-status-title"><?= htmlspecialchars($orderNumber) ?></h1>
                            </div>
                        ...
                        <section class="order-detail-grid" aria-label="Order information">
                            <dl class="order-detail-list">
                                ...
                                <div>
                                    <dt>Phone</dt>
                                    <dd><?= htmlspecialchars($order['user_phone'] ?: 'N/A') ?></dd>
                                </div>
                                <div>
                                    <dt>Order Type</dt>
                                    <dd><?= htmlspecialchars($orderTypeLabels[$typeKey] ?? ucfirst($typeKey)) ?></dd>
                                </div>
                                <div>
                            ...
                            <dl class="order-detail-list">
                                <div>
                                    <dt>Order Time</dt>
                                    <dd><?= htmlspecialchars($createdAt->format('F d, Y g:i a')) ?></dd>
                                </div>
                                <div>
                                    <dt><?= htmlspecialchars($addressLabel) ?></dt>
                                    <dd><?= htmlspecialchars($addressValue) ?></dd>
                                </div>
                                <div>
                                    <dt>Total Amount</dt>
                        ...
                        <section class="order-progress" aria-labelledby="progress-title">
                            <div class="status-section-heading">
                                <h2 id="progress-title">Order Status</h2>
                                <p>Update the status of this order</p>
                            </div>

                            <ol class="status-track" aria-label="Order progress">
                                <?php foreach ($steps as $index => $step): ?>
                                    <?php
                                    $isCurrent = $index === $statusIndex && !in_array($orderStatus, ['completed', 'cancelled'], true);
                                    $className = pos_status_step_class($stepReached[$index], $isCurrent);
                                    ?>
                                    <li class="<?= $className ?>" <?= $isCurrent ? ' aria-current="step"' : '' ?>>
                                        <span class="step-dot"><i class="fa-solid <?= htmlspecialchars($step['icon']) ?>"></i></span>
                                        <span class="step-title"><?= htmlspecialchars($step['title']) ?></span>
                                        <?php if ($stepReached[$index] && $createdAt): ?>
                                            <time><?= htmlspecialchars($createdAt->format('g:i A')) ?></time>
                                        <?php endif; ?>
                                        <?php if ($isCurrent && $step['note'] !== ''): ?>
                                            <span class="step-note"><?= htmlspecialchars($step['note']) ?></span>
                                        <?php endif; ?>
                                    </li>
                                <?php endforeach; ?>
                            </ol>
                            ...
                            <div class="status-alert" role="status">
                                ...
                                <div>
                                    <p><?= htmlspecialchars($statusMessage[0]) ?></p>
                                    <span><?= htmlspecialchars($statusMessage[1]) ?></span>
                                </div>
                            </div>
                        </section>
    ...
    <script>
        const currentOrderId = Number(<?= json_encode((int) ($order['id'] ?? 0)) ?>);

        document.querySelectorAll('.status-actions [data-status]').forEach((button) => {
            button.addEventListener('click', async () => {
                const nextStatus = button.dataset.status;
                if (!currentOrderId || !nextStatus || button.disabled) return;
